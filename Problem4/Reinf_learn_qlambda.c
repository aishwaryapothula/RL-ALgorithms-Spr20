...
double lamda = 0.9;
int a;
int Q_learn(reward,explore)
double reward;
int *explore;
{  
  float opimtal_future_value;
  int new_action = 1;
  

  if (Q_values[0][prev_state]  < Q_values[1][prev_state])
      a = 1;
  else
      a = 0;
  double delta = reward + gamma * Q_values[new_action][prev_state] - Q_values[a][prev_state];
  Errors[prev_action][prev_state] ++;
  
  for(int act = 0; act < ACTIONS; act++){
    for(int stat = 0; stat < STATES; stat++){
      Q_values[act][stat] += beta * delta * Errors[act][stat];
      if(new_action == a){
        Errors[act][stat]  = gamma * lamda * Errors[act][stat];
      }else{
        Errors[act][stat] = 0;
      }
 ...
 // trial in the next time step once the pendulum has fallen
  ...
  if ( reset )
  {
    ...
    *explore = 0;
    ...
    prev_state = Decoder3Dn(0, pole, &invert[0], fail);
    prev_action = action;
    action = Q_learn(0.0 - *fail,explore);
    if ( invert[0] )
      force[0] = (double)(2 * action - 1) * -F_X;
    else
      force[0] = (double)(2 * action - 1) * F_X;
  }
  force[1] = 0.0;
