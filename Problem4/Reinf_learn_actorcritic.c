...
return take_action;
 ...
 double actor_critic(int reset, double reward)
{
    double c_pol, a_pol, sum;
    double alpha = 0.4;
    double beta = 0.1;
    double gamma = 0.6;
    double Advantage;
  ...
  if (!reset && !(prev_state == -1))
  {
   ...
   else
   {
      ...
      double advantage = reward + gamma * new_vs - VS[prev_state];

 VS[prev_state] += Alpha * (reward + gamma * new_vs - VS[prev_state]);
      policy[prev_action][prev_state] += beta * Advantage;
      
      
   if(new_state != -1)
   {
       c_pol = policy[new_state][0];
       a_pol = policy[new_state][1];
       double sum = c_pol + a_pol;
       c_pol = exp(c_pol)/sum;
       a_pol = exp(a_pol)/sum;
   }
      return c_pol,a_pol;
  ...
  // Example of computing the reward from the fail flag computed by the decoder
    double reward;
        ...
        new_state = Decoder3Dn(0, pole, &invert[0], fail);
            ...
            reward = 0.0;
p_c, p_a = actor_critic(reset, reward);
if (new_state != -1){
 if(r < p_c)
    new_action = 0;
  else
