...
{
int take_action;
 if(Q_table[0][new_state] > Q_table[1][new_state]){
          take_action =0;
        }else{
          take_action =1;
        }
...
int Sarsa(double reward, int *explore ){
    //printf("oa: %d,Os: %d,ns: %d,rf: %f,Q: %f\n",action,prev_state,new_state,reward,Q_table[action][prev_state]);
	...
	take_action = argmax(new_state);
    } 
Q_table[prev_action][prev_state] += Alpha * (reward + Gamma * Q_table[take_action][new_state] - Q_table[prev_action][prev_state]);

return take_action;
}

int Q_learning(double reward, int *explore ){
    
    double r = rand()/(double)RAND_MAX;
    int take_action;
    
    Q_table[prev_action][prev_state] += Alpha * (reward + Gamma * Q_table[argmax(new_state)][new_state] - Q_table[prev_action][prev_state]);
  ...
  // Example of computing the reward from the fail flag computed by the decoder
	...
	prev_action = action;
        new_state = Decoder3Dn(0, pole, &invert[0], fail);
            ...
            reward = 0.0;
        action = Sarsa(reward, explore);
        if (invert[0])
            force[0] = (2 * action - 1) * -10.0;
        else
            force[0] = (2 * action - 1) * 10.0;
    }
    force[1] = 0.0;
